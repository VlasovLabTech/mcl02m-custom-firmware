...
- APPROACH: below the first topology boundary; current formula `8 + 2×error`,
  capped at 35.
- HOLD: conservative PI controller capped at gear 35.
...
- These controller constants remain unchanged. A real cooking run showed that
  the lab-only 120 °C bottom-sensor guard was inappropriate for production; it
  is disabled there while the stock power MCU's native E05 remains active.
...
The previously built `0.2.4-dev` app was 877,488 bytes and had SHA-256
...
- The owner expanded the selectable range to 40–190 °C after supervised
  real-use tests. PI gains and preheat/approach behavior remain unchanged.
- Production retains the 80 °C interface-side IGBT guard. The former 120 °C
  bottom-sensor guard was a lab bring-up limit and is disabled in production;
  the power MCU's native E05 remains authoritative.
- Investigate the observed high-power E09 by capturing per-transaction I²C
  errors and checking wiring, pull-ups, supply integrity, and EMI coupling.
