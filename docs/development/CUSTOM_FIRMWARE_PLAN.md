...
Испытательные пределы 80 °C IGBT и 120 °C bottom NTC работали как
консервативные границы стендового запуска. Практическая готовка показала, что
bottom guard 120 °C непригоден для production, поэтому там он отключён;
штатная E05 силового контроллера остаётся активной. Production IGBT guard 80 °C
оказался ошибочным переносом стендового лимита. Текущая логика: после двух
достоверных измерений выше 92 °C непрерывно действует предупреждение без снижения
мощности, которое сохраняется при 92 °C и снимается строго ниже 92 °C; после двух
измерений выше 98 °C возникает помеченная интерфейсная E07, а новый Start запрещён
выше 80 °C. Bottom guard требует шесть последовательных измерений строго выше
210 °C.
