...
- `POST /api/profile`: `index`, `name` и для каждой ячейки `1…5` поля
  `modeN`, `gearN` (`0…99`), `tempN` (`40…190`) и `timeN` в минутах.
  A timed POWER cell with gear 0 is an active-zero wait cell; `timeN=0`
...
Cooking timer, delayed Start, active/fault/LED state и telemetry во flash не пишутся.
Persistent writes разрешены только модулю `settings.c` после явного Save. Faults
therefore cannot persist a disabled LED state through the custom NVS namespace.
