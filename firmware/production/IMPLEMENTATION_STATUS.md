...
eFuse и силовая firmware не изменялись. Владелец подтвердил нормальный boot и
работу пользовательского интерфейса этой версии. Повторная метрологическая
калибровка NTC/PI и полный набор длительных силовых испытаний всё ещё остаются
отдельным supervised-этапом.
...
| TEMPERATURE | provisional `40…175 °C`; до Start уставка `S…°` и текущий NTC показываются двумя строками ×2; во время работы/паузы вращение encoder временно включает тот же экран на 2 s после последнего шага; PREHEAT `56/77/99`, APPROACH/HOLD не выше `35` |
...
## Не подтверждено без supervised hardware validation

- Абсолютная калибровка обоих NTC и пригодность provisional диапазона
  `40…175 °C`.
- Безопасные дополнительные interface guards `IGBT 80 °C / bottom 120 °C`:
  силовая плата имеет собственную защиту, но эти значения надо сверить на стенде.
- Коэффициенты PI, переходы PREHEAT/APPROACH/HOLD, overshoot и критерий
  `HOLD SATURATED`.
- Реальное поведение всех fault paths и стабильность web при длительном нагреве.

До этих проверок firmware остаётся тестовой и не предназначена для автономного
приготовления пищи.
