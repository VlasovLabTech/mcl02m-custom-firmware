...
Дата: 2026-09-09
Версия исходников: `0.2.38-dev`
Статус: reference public/private `0.2.38-dev` artifacts are built and hash-recorded.
The immediately preceding private 2.36 artifact was written only to stock `ota_1`
at `0x170000` on the development unit on 2026-09-08. The current label-only rebuild
and the public artifact remain unflashed. Supervised testing
...
| OLED | renderer использует полные `64×48` без отладочной рамки; 19 compiled frames include updated turn-on/wake/cooking/confirm/cancel/no-pan/error/Sleep artwork, three Ready variants rotated per completion, Wi-Fi, Hot, Time and Small plus three reserved inactive frames; Ready auto-closes after one minute, then Hot/OLED/Sleep policy resumes; Hot appears after five inactive seconds above 60 °C and blinks 2 s on / 1 s blank; a physical press/turn dismisses an existing timed picture before its own action; active timeout default 3 min; `SCREEN ON → COOK MODE` (`ЭКРАН ПРИ → ГОТОВКЕ ВКЛ`) optionally keeps OLED on only in active Start/Cooking/Pause/NoPan/Stopping states; первое нажатие/вращение только будит; encoder guard 1,5 s |
...
| Readings | INFO: три строки `VOLT`, `NTC`, `IGBT`; сверху рабочего POWER — NTC, сверху TEMPERATURE — `S…°` и power; English-only `DEBUG SHOW R21` optionally adds raw decimal `R21` as `Rxxx`; English-only `SHOW DEBUG BAD I2C CNT` independently adds the current-session critical-I²C total as `B000…B999`; without a timer the corner cycles 5 s context and 2 s for each enabled debug value; timer and Pause suppress diagnostic phases; both values are display-only |
...
| I²C recovery/debug | `R20/R22/R23/R24/R26` and `0D/00/0C` writes are critical; `R21/R25/R27` are service-only. Three critical-bad cycles enter a 320-ms critical-only poll, two complete good cycles restore the 500-ms poll, and E09 requires 15 s continuous critical loss or 10 s continuous command-write loss. `Bxxx` accumulates every critical-bad cycle from the pre-Start baseline through the cooking session, does not reset on recovery, saturates at 999 and resets at the session boundary. The first-cause RAM incident and compact/authenticated diagnostics remain active. The older consecutive `0…6` peak-hold overlay remains compiled out with `COOKER_I2C_DEBUG_DISPLAY_ENABLED=0` |
